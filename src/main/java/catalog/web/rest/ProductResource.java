...
import catalog.web.rest.util.HeaderUtil;
import catalog.web.rest.util.PaginationUtil;
...
public class ProductResource {
    ...
    /**
     * GET  /catproducts : get all the products by catalog id.
     *
     * @param idCat the pagination information
     * @param pageable the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of products in body
     */
    @GetMapping("/catproducts/{idCat}")
    @Timed
    public ResponseEntity<List<Product>> getAllProductsByCategory(@ApiParam Pageable pageable, @PathVariable Long idCat) {
        log.debug("REST request to get a page of Products");
        Page<Product> page = productService.findAllByCategory(pageable, idCat);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/catproducts/{idCat}");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
    ...
    public ResponseEntity<Product> getProduct(@PathVariable Long id) {
        ...
        Product product = new Product(productService.findOne(id));
